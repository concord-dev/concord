...
import (
	...
	"github.com/concord-dev/concord/internal/evidence/github"
...
func TestGitHubCollector_PropagatesUnauthorized(t *testing.T) {
	...
	c := github.New("bad").SetBaseURL(srv.URL)
...
func TestGitHubCollector_FileGlob_FrontmatterParse(t *testing.T) {
	...
	c := github.New("t").SetBaseURL(srv.URL)
	v, err := c.Collect(evidence.Context{}, apiv1.EvidenceRef{
...
func TestGitHubCollector_Probe(t *testing.T) {
	...
	bad := github.New("nope").SetBaseURL(srv.URL)
