...
and pins the digest + signer in concord.lock.`,
	...
	cmd.Flags().StringVar(&opts.LockfilePath, "lockfile", plugins.LockfilePath, "Path to concord.lock")
...
func newPluginListCmd() *cobra.Command {
	...
	cmd.Flags().StringVar(&lockPath, "lockfile", plugins.LockfilePath, "Path to concord.lock")
	return cmd
}
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, p := range lf.Plugins {
				...
				_, err := plugins.VerifySignature(ctx, ref, plugins.VerifyOptions{Identity: p.Signer, CosignBin: cosignBin})
				if err != nil {
					fail++
