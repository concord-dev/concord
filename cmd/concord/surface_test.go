...
// The ~21 GRC-admin CRUD verbs live as HIDDEN aliases under `admin` and are
// excluded here by design. Adding a NEW visible top-level verb must be a
// deliberate act: prefer making it an `admin` verb, a collector plugin, a
...
var wantVisibleTopLevel = []string{
	...
	// Administration — the single door to GRC program management.
	"admin",
}
